...
1. What currently lives only in the owner's head vs. what is written down anywhere
2. Current team size and realistic hiring plans for the next 6-12 months
3. Where things are breaking down, falling through the cracks, or requiring the owner's direct involvement to not go wrong
4. What onboarding has looked like when they have brought someone new on
5. Seasonal or operational pressures that are specific to their situation
6. Where the owner's time is going and where they most need to stop being the bottleneck
7. Any near-term events that change the urgency — a new hire starting, a busy season coming, a key person leaving
...
async function callClaude(prompt, onChunk) {
  const response = await fetch("https://api.anthropic.com/v1/messages", {
    ...
    body: JSON.stringify({
      ...
      max_tokens: 4000,
      stream: true,
      tools: prompt.includes("website URL:") ? [{
...
export default function SOPBuilder() {
  ...
  const run = async (promptText, onDone, label) => {
    setLoading(true);
    setStreamText("");
    setError("");
    try {
  ...
  const runPrompt1 = () => {
    ...
    setStep("brief");
    run(prompt, (result) => { setBrief(result); }, "Business Brief");
  };

  const runPrompt2 = () => {
    setStep("discovery");
    run(PROMPTS.discoveryQuestions(brief), (result) => { setDiscovery(result); }, "Discovery Questions");
  };

  const runPrompt3 = () => {
    setStep("plan");
    run(PROMPTS.sopPlan(brief, answers), (result) => { setPlan(result); }, "SOP Plan");
  };

  const runPrompt4 = () => {
    setStep("done");
    run(PROMPTS.writeSop(brief, sopTitle), (result) => { setFinalSop(result); }, "Finished SOP");
  };
    ...
    <div style={{
      ...
      <style>{`
        ...
        * { box-sizing: border-box; }
        ...
        textarea::placeholder, input::placeholder { color: ${COLORS.muted}; opacity: 0.6; }
        textarea:focus, input:focus { border-color: ${COLORS.accentDim} !important; }
      ...
      <div style={{ maxWidth: 760, margin: "0 auto" }}>
          ...
          <div style={{ background: COLORS.surface, border: `1px solid ${COLORS.border}`, borderRadius: 12, padding: 32 }}>
            ...
            {inputMode === "url" ? (
              ...
              <>
                ...
                <Input label="Services Offered" value={manualData.services} onChange={v => setManualData(p => ({ ...p, services: v }))} placeholder="Weekly mowing, seasonal cleanups, snow removal triggered at 3 inches..." multiline rows={3} />
        ...
        {step === "brief" && (
          <div style={{ background: COLORS.surface, border: `1px solid ${COLORS.border}`, borderRadius: 12, padding: 32 }}>
            ...
            <p style={{ color: COLORS.muted, fontSize: 14, margin: "0 0 4px", lineHeight: 1.6 }}>Prompt 1 complete. Review the brief below before generating discovery questions.</p>
        ...
        {step === "discovery" && (
          <div style={{ background: COLORS.surface, border: `1px solid ${COLORS.border}`, borderRadius: 12, padding: 32 }}>
            ...
            <p style={{ color: COLORS.muted, fontSize: 14, margin: "0 0 4px", lineHeight: 1.6 }}>Prompt 2 complete. Use these questions on your discovery call or answer them yourself below.</p>
            ...
            {!loading && discovery && (
              <div style={{ marginTop: 24 }}>
                ...
                <p style={{ color: COLORS.muted, fontSize: 13, margin: "0 0 12px", lineHeight: 1.6 }}>Paste your call notes or answer the questions above. This goes into Prompt 3.</p>
                <textarea
                  value={answers}
                  onChange={e => setAnswers(e.target.value)}
                  placeholder="Paste discovery call notes or type answers here..."
                  rows={10}
                  style={{
                    width: "100%", background: COLORS.bg, border: `1px solid ${COLORS.border}`,
                    borderRadius: 6, padding: "12px 14px", color: COLORS.text,
        ...
        {step === "plan" && (
          <div style={{ background: COLORS.surface, border: `1px solid ${COLORS.border}`, borderRadius: 12, padding: 32 }}>
            ...
            <p style={{ color: COLORS.muted, fontSize: 14, margin: "0 0 4px", lineHeight: 1.6 }}>Prompt 3 complete. Review the full SOP plan. Then pick a Phase 1 SOP to write first.</p>
            ...
            {!loading && plan && (
              <div style={{ marginTop: 24 }}>
                ...
                <input
                  ...
                  placeholder="e.g. Daily Crew Launch and Morning Routine"
                  style={{
                    width: "100%", background: COLORS.bg, border: `1px solid ${COLORS.border}`,
                    borderRadius: 6, padding: "12px 14px", color: COLORS.text,
        ...
        {step === "done" && (
          <div style={{ background: COLORS.surface, border: `1px solid ${COLORS.border}`, borderRadius: 12, padding: 32 }}>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 8 }}>
              ...
              {finalSop && !loading && (
                <button onClick={() => copyToClipboard(finalSop)} style={{ background: COLORS.accentDim, border: "none", borderRadius: 6, padding: "8px 16px", color: "#fff", cursor: "pointer", fontSize: 13, fontWeight: 600, fontFamily: "inherit" }}>
                  Copy SOP
                </button>
              )}
            </div>
